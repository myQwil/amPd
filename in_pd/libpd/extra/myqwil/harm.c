...
EXTERN t_float ntof(t_float f, double r, double s);
...
int harm_limtr(t_harm *x, int n, int i) {
...
static void harm_set(t_harm *x, t_symbol *s, int ac, t_atom *av) {
	...
	{	int i = harm_limtr(x, av->a_w.w_float, 0);
...
double harm_getnote(t_harm *x, int d) {
...
static void harm_float(t_harm *x, t_float f) {
	int n=x->x_in, d=f;
	double note = harm_getnote(x, d);
	if (f!=d)
	{	int b = f<0?-1:1;
		double next = harm_getnote(x, d+b);
